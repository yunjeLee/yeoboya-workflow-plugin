...
- **인자 없음.** 항상 8 개 핵심/부가 하네스 문서 전체를 검증한다.
- 호출 전 8 개 파일 존재 확인은 호출자(SKILL) 의 책임. 부재 시 호출자에서 종료. 이 모듈은 8 개가 모두 존재한다고 가정한다.
...
- **부가 2 종 (존재 시만)**: `docs/UI_GUIDE.md`, `docs/WORKFLOW.md`
