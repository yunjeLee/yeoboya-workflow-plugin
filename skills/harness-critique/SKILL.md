...
이미 생성된 하네스 문서 세트(8 개) 를 5 축으로 검증한다. `/harness` 또는 `/harness-edit` 안에서도 자동 호출되지만, 이 명령은 **수정 없이 검증만** 따로 부르고 싶을 때 진입점이다 (예: 팀원이 IDE 로 직접 편집한 후 점검).
...
> 인자 없음. 항상 8 개 전체 (핵심 6 + 부가 2 의 존재분) 검증.
