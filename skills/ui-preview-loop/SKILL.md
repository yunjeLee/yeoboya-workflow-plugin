...
`shared/ui-review.md`의 피드백 루프 지침을 따른다.

매 수정 후 아래 메시지를 출력하고 사용자 응답을 기다린다:
...
응답에 따라 처리:
- **스크린샷 첨부 또는 텍스트 설명**: 문제를 분석하고 수정 후 루프를 반복한다. 회차를 1 증가시킨다.
- **0 입력**: Step 4(인계)로 진행한다.
- **y 입력**: Step 4(완료)로 진행한다.
