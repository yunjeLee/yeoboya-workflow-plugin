...
## Step 4: /onboard 자동 연계

스캐폴딩(`/phase` 전체 실행)이 완료된 후 `/onboard` 스킬을 자동 실행하여 하네스 문서 세트를 생성한다. 이 시점에는 핵심 문서가 아직 없으므로 `/onboard` 는 Case 1 (전체 생성) 로 진입한다.

`/onboard` 종료 후 사용자에게 다음을 안내한다:
- 생성된 docs/ 파일 목록
- 이후 `/feature`, `/bugfix` 등을 실행하면 자동으로 하네스 가드레일이 적용됨

> **주의**: 스캐폴딩이 실패 또는 중단된 경우 `/onboard` 를 호출하지 않는다 (실제 코드가 없으면 자동 스캔 결과가 비어 있어 `[TBD]` 만 잔뜩 생성됨). 사용자에게 "스캐폴딩 완료 후 `/onboard` 를 수동 실행하세요" 안내로 대체.
...
- 구현은 /phase로 단계별 실행
- 구현 완료 후 /onboard 자동 실행 → 하네스 문서 세트 생성
```
