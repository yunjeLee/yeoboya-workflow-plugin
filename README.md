...
팀 단위로 Claude Code 를 쓰면 마주치는 2 가지 문제가 있다.
...
이 plugin 은 **하네스 문서 세트** 로 위 문제를 해결한다. `CLAUDE.md` + `docs/` 6 개
문서. 팀이 한 번 작성해두면 Claude 가 세션 시작 시 자동으로 읽고, spec-driven-development
의 사양 역할을 한다.
...
### `/harness-module [모듈]` — 모듈 단위 CLAUDE.md 생성
...
인자로 모듈명 (`:feature:home` / `Home`) 을 넘기면 자동 추천을 우회하고 그 모듈만
처리한다. 생성 직후 `verify-module-docs` 로 4 축 검증 (consistency / referential
integrity / reality-check / 50 줄 한도) 1 회 실행.

---

### `/harness-module-edit {모듈}` — 모듈 CLAUDE.md 부분 수정

이미 생성된 `{path}/CLAUDE.md` 를 git diff 기반으로 갱신한다. 진입 파일이 바뀌면
"역할" 섹션을, 선언 파일 의존성이 바뀌면 "의존성" 섹션을 자동 추천 후보로 올린다.
사용자가 [y/N] 으로 섹션마다 승인. 다른 섹션은 절대 건드리지 않는다.

수정 후 `verify-module-docs` 로 검증 1 회 실행.
