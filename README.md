...
부분 수정은 `/harness-edit {모듈명}`, 검증은 `/harness-critique` 로 책임이
분리되어 있습니다. 신규 합류자 진입점은 `CLAUDE.md` 자체입니다.
...
**v2.4.3 추가 변경:**
- `/onboard` 폐기. 하네스 문서 8 종을 펼쳐 요약하는 브리핑은 정보 과부하이며,
  신규 합류자 진입점은 `CLAUDE.md` 자체로 충분합니다.
